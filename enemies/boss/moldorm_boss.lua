  ...
  if enemy:get_life() < 2 then
    ...
    enemy:start_death(function()
      ...
      sol.timer.start(enemy, 2000, function()
        enemy:start_sprite_explosions(sorted_tied_sprites, "entities/explosion", 0, 0, function()
          sol.timer.start(enemy, 1000, function()
            local x, y = head_sprite:get_xy()
            enemy:start_brief_effect("entities/explosion", nil, x, y)
            finish_death()
          end)
        end)
      end)
...
enemy:register_event("on_created", function(enemy)
 ...
 -- sprites[5] = enemy:create_sprite("enemies/boos/moldorm/moldorm_tail")
  for i = 3, 1, -1 do
    sprites[i + 1] = enemy:create_sprite("enemies/boss/moldorm/moldorm_body_" .. i)
  end
