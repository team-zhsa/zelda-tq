...
function inventory_submenu:on_started()
  ...
  if self.game:has_item("magic_powders_counter") then
    item_names_assignable2 [2] = "magic_powders_counter"
  else
    item_names_assignable2 [2] = "mushroom"
  end
  if self.game:get_value("get_boomerang") then
    item_names_assignable2 [8] = "boomerang"
  end
  ...
  -- Load Items
  for i,item_name in ipairs(item_names_assignable2) do
  ...
  for i,item_name in ipairs(item_names_static2 ) do
    local item = self.game:get_item(item_name)
    local variant = item:get_variant()
...
function inventory_submenu:on_draw(dst_surface)
  ...
  for j = 0, 3 do
    k = k + 1
    local item = self.game:get_item(item_names_static2[k])
  ...
  for i = 0, 3 do
    ...
    for j = 0, 2 do
      k = k + 1
      if item_names_assignable2 [k] ~= nil then
        local item = self.game:get_item(item_names_assignable2 [k])
...
function inventory_submenu:on_command_pressed(command)
  ...
  if not handled then
    ...
    elseif command == "down" and self.cursor_column < 5 then
      sol.audio.play_sound("cursor")
...
function inventory_submenu:set_cursor_position(row, column)
  ...
  local item = item_name and self.game:get_item(item_name)
...
function inventory_submenu:get_item_name(row, column)
    ...
    if column > 0 and column < 4 then
      ...
      item_name = item_names_assignable2 [index + 1]
   ...
   elseif column == 6 then
      ...
      index = row 
      item_name = item_names_static2 [index + 1]
  end
